...
//Вычисление окна времени.
func parseTimeWindowLimit(rawLimit uint64, timeMult float64) float64 {

	//Взято из Intel SDM, Vol 3B 14.10.3
	/*
		Time limit = 2^Y * (1.0 + Z/4.0) * Time_Unit
		“Y” - unsigned integer представленное в битах 21:17,
		“Z” - unsigned integer представленное в битах 23:22.
		“Time_Unit” определяется “Time Units” полем из MSR_RAPL_POWER_UNIT.
	*/
...
//MSR_RAPL_POWER_UNIT MSR
func parsePowerUnit(msr uint64) RAPLPowerUnit {

	var powerUnit RAPLPowerUnit
	//Значения из MSR обработанные как регистры согласно SDM
